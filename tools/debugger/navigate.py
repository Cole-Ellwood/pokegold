...
class CheckpointStaleError(RuntimeError):
    """A seed checkpoint's recorded basis no longer matches the live files.

    Restoring an emulator save-state onto a ROM it was not captured against
    executes mismatched code and silently observes garbage ("no map loaded"),
    so the loader refuses with the mismatch named instead of navigating.
    """


BOSS_CAPTURE_REFRESH_COMMAND = (
    "python -m tools.boss_ai_debugger run-suite --profile changed-ai "
    "--rebuild-roms --refresh-live-traces"
)


def require_checkpoint_basis(name: str, *, path: Path, recorded_sha256: str) -> None:
    """Fail closed when a seed checkpoint's dependency drifted from its manifest."""
    if not path.exists():
        raise CheckpointStaleError(
            f"checkpoint {name!r} basis file missing: {path}; "
            f"re-run: {BOSS_CAPTURE_REFRESH_COMMAND}"
        )
    if not recorded_sha256:
        return
    live = log_sha256(path)
    if live.lower() != recorded_sha256.lower():
        raise CheckpointStaleError(
            f"checkpoint {name!r} is stale: {path.name} sha256 mismatch "
            f"(manifest {recorded_sha256[:12].lower()}..., live {live[:12]}...); "
            f"the seed state was captured against a different build -- "
            f"re-run: {BOSS_CAPTURE_REFRESH_COMMAND}"
        )
...
def load_boss_ai_checkpoint(capture_id: str) -> dict[str, Any]:
    ...
    checkpoint_id = BOSS_AI_CHECKPOINT_PREFIX + capture_id.lower()
    require_checkpoint_basis(
        checkpoint_id, path=trace_rom, recorded_sha256=str(data.get("trace_rom_sha256") or "")
    )
    require_checkpoint_basis(
        checkpoint_id,
        path=trace_symbols,
        recorded_sha256=str(data.get("trace_symbols_sha256") or ""),
    )
    manifest = {
        "schema_version": 1,
        ...
        "source_manifest": str(BOSS_TRACE_MANIFEST.relative_to(ROOT) if BOSS_TRACE_MANIFEST.is_relative_to(ROOT) else BOSS_TRACE_MANIFEST),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.verify:
        try:
            ...
            print(f"stale checkpoint basis: {exc}", file=sys.stderr)
            return 2
        if args.json:
            print(json.dumps(report, indent=2))
        else:
