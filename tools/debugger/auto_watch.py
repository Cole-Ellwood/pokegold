...
detectors and emits findings to ``audit/auto_watch_findings.jsonl`` so the
Claude+Codex pair sees regressions without having to ask.
...
- The ``--on rom-edit-propose`` / ``--on commit`` triggers (Codex's hook
  installer lands the post-commit trigger plumbing in parallel under
