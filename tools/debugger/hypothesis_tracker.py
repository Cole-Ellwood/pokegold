...
V0 scope: persist hypotheses across Claude+Codex sessions with citation
...
Confidence labels (mirror ``docs/llm_pairing_rules.md`` rule #4)
----------------------------------------------------------------
...
This module is import-friendly and provides a CLI entry point via
``python -m tools.debugger.hypothesis_tracker``. CLI wiring into the
top-level ``python -m tools.debugger`` is held pending a sync with the
Codex lane (per the collision-risk file list in
