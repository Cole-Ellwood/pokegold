...
def build_capability_report(root: Path = ROOT) -> dict[str, Any]:
    capabilities = [
        ...
        _capability(
            ...
            gaps=(
                "Watch replay reports changes, deterministic supported text input-log playback, preceding frame context, PC/register snapshots, bounded static source-cause candidates, PyBoy hook-order self-checks for pre-instruction RMW byte attribution, investigation-time executed hook-order validation before fresh attribution trace capture, dynamic-taint sink-write attribution from instruction traces and effect-trace reports including modeled byte values and direct stack-write attribution for taken call/rst/push instructions, bounded instruction effect traces with reads/writes/known byte values/value-source provenance/stack/IO/control-flow, explicit evidence-source labels, pre-instruction register and observed-memory snapshots on effect events, adjacent-frame post-value validation for modeled writes, adjacent-frame observed byte changes without modeled writers, common hardware side-effect trigger indexes, modeled OAM DMA copy reads/writes with runtime bank metadata for ROMX/WRAMX/SRAM sources when observed, observed general-mode CGB VRAM DMA copy reads/writes, observed adjacent-frame TIMA overflow reload/IF timer-interrupt request effects, trace-proven interrupt-entry stack writes, pre-instruction trace-window checkpoints, modeled checkpoint-to-writer effect-span consistency, and reverse-query last-writer answers with validation routes back to the source effect report or instruction trace, but full reverse execution across every CPU side effect is still not implemented.",
            ),
            commands=(
