...
NEXT_STEP_ROWS = [
    ...
    },
    {
        ...
        "keywords": ["wrong switch", "bad switch", "selected switch", "boss switch", "boss switched", "switch into", "switch out", "propose a switch", "proposed switch", "switch_sack", "expected staying", "expected to stay", "stay in", "staying in", "should stay", "switched when expected", "policy expected", "switch confidence", "switch target", "preserve"],
    ...
    },
    {
        ...
        "keywords": ["wrong move", "bad move", "move score", "scoring", "selected move", "score mismatch", "ai move", "used curse", "uses curse", "too eager to curse", "pain split", "used pain split", "uses pain split", "instead of"],
...
DISPROOF_STANDARDS = {
    ...
    "wrong_switch": [
        "If a matching scenario JSONL passes rom-switch-materialize with the expected switch result, the wrong-switch claim is disproven for that scenario. If rerunning the same switch_sack scenario no longer reports the disagreement, if switch_roll.available=false because no switch dispatch observation was reached, if the trace-ROM/symbol hashes mismatch, if the base state is already inside/past BossAI_TrySwitch, or the answer cannot connect the policy card to BossAI_TrySwitch behavior, this candidate should not be treated as proven. If the returned route omits rom-switch-materialize or routes to damage/banking fallbacks instead of the boss_policy_switch surface, the oracle failed this question.",
    ],
    "wrong_move_score": [
