...
EVIDENCE_STANDARDS = {
    ...
    "headless_battle_simulation": [
        "The answer routes to tools.headless_battle, uses a JSON state/actions-or-turns-or-repeat/RNG scenario, and preserves proof labels: pre-variation damage is delegated to the existing ROM-backed damage oracle, selected switches and caller-supplied replacements carry active/bench state, explicit enemy auto_replace uses the source-shaped basic type-chart replacement choice, auto_replace_or plus repeat/max_turns can run simple KO/send-out loops, wild_random_move is source-mirrored, PP decrement, explicit active HP restore item actions, explicit stat-stage state, selected BP=0 single-stat stage moves, selected Dragon Dance/Calm Mind/Quiver Dance setup moves, selected Recover/Softboiled/Milk Drink self-heal moves, selected PoisonPowder/Poison Gas/Toxic poison-status moves, selected Thunder Wave/Stun Spore/Glare paralysis-status moves, selected damaging burn/poison/paralysis status secondaries, selected Absorb/Mega Drain/Leech Life/Giga Drain drain moves, and supported after-hit item HP effects are covered, modified-speed order, paralyzed speed/full-paralysis action denial including Fighting/Electric type-passive paralysis modifiers, non-link equal-speed ties, basic critical hits, basic move accuracy, damage variation, selected damaging status secondary chance, and auto-replace fallback have fixed/sample/exhaustive RNG branching where applicable, initial poison/burn/toxic residual is source-mirrored after selected moves and supported item actions, post-score Boss AI selector replay/execution starts from known score bytes, and unimplemented strategic/full-battle mechanics remain out of scope.",
    ],
    "type_chart_navigation": [
