...
NEXT_STEP_ROWS = [
    ...
    },
    {
        ...
        "proof_limit": "Current headless slice: selected move-vs-move turns, selected BP=0 single-stat stage moves, selected Recover/Softboiled/Milk Drink self-heal moves, selected PoisonPowder/Poison Gas/Toxic poison-status moves, selected Thunder Wave/Stun Spore/Glare paralysis-status moves with baseline paralyzed speed/full-paralysis checks, repeat/max_turns action maps, auto_replace_or KO/send-out loops, wild_random_move, selected switch actions, caller-supplied replace actions after KO, explicit enemy auto_replace choice using the basic source type chart, explicit active Potion/Super Potion/Hyper Potion/Max Potion/Full Restore actions, explicit attack/defense/speed/sp_attack/sp_defense stat-stage state, PP decrement, supported Rocky Helmet/Shell Bell/Life Orb after-hit HP effects, modified-speed turn order, non-link equal-speed tie RNG, basic critical-hit checks, basic move accuracy hit/miss, damage variation with fixed/sample/exhaustive RNG, initial poison/burn/toxic residual after selected moves and supported item actions, multi-turn turns[] progression with HP/RNG carryover, pre-variation damage delegated to the existing ROM-backed damage oracle, and post-score Boss AI selector replay/execution from known score bytes. Strategic full-battle action choice, automatic trainer item usage, player trainer-battle Pack availability, inventory accounting, implicit replacement without auto_replace_or or auto_replace, Pursuit/Spikes/switch-in effects, accuracy/evasion stage moves/modifiers, damaging secondary stat effects, multi-stat chain moves, Baton Pass/Psych Up, Substitute/Mist blockers, badge boosts, passive stat/speed/accuracy bonuses, status application outside selected poison/paralysis-status moves, sleep/freeze, burn application, Safeguard/Substitute, held status prevent/cure item consumption, Fighting/Electric type-passive paralysis modifiers, Rest, weather/time healing, drain moves, Heal Bell, RNG-consuming mechanics outside the listed supported branch points, live Boss AI scoring, and scripts remain out of scope until separately implemented and proven.",
...
EVIDENCE_STANDARDS = {
    ...
    "headless_battle_simulation": [
        "The answer routes to tools.headless_battle, uses a JSON state/actions-or-turns-or-repeat/RNG scenario, and preserves proof labels: pre-variation damage is delegated to the existing ROM-backed damage oracle, selected switches and caller-supplied replacements carry active/bench state, explicit enemy auto_replace uses the source-shaped basic type-chart replacement choice, auto_replace_or plus repeat/max_turns can run simple KO/send-out loops, wild_random_move is source-mirrored, PP decrement, explicit active HP restore item actions, explicit stat-stage state, selected BP=0 single-stat stage moves, selected Recover/Softboiled/Milk Drink self-heal moves, selected PoisonPowder/Poison Gas/Toxic poison-status moves, selected Thunder Wave/Stun Spore/Glare paralysis-status moves, and supported after-hit item HP effects are covered, modified-speed order, baseline paralyzed speed/full-paralysis action denial, non-link equal-speed ties, basic critical hits, basic move accuracy, damage variation, and auto-replace fallback have fixed/sample/exhaustive RNG branching where applicable, initial poison/burn/toxic residual is source-mirrored after selected moves and supported item actions, post-score Boss AI selector replay/execution starts from known score bytes, and unimplemented strategic/full-battle mechanics remain out of scope.",
    ],
    "type_chart_navigation": [
...
DISPROOF_STANDARDS = {
    ...
    "headless_battle_simulation": [
        "If the report claims strategic automatic full-battle action choice, automatic trainer item usage, player trainer-battle Pack availability, inventory accounting, implicit replacement without auto_replace_or or auto_replace, Pursuit/Spikes/switch-in effects, link-battle turn-order inversion, accuracy/evasion stage moves/modifiers, damaging secondary stat effects, multi-stat chain moves, Baton Pass/Psych Up, Substitute/Mist blockers, badge boosts, passive stat/speed/accuracy bonuses, status application outside selected poison/paralysis-status moves, sleep/freeze, burn application, Safeguard/Substitute, held status prevent/cure item consumption, Fighting/Electric type-passive paralysis modifiers, Rest, weather/time healing, drain moves, Heal Bell, RNG-consuming mechanics outside the listed supported branch points, live Boss AI scoring, scripts, or byte-proven turn sequencing beyond the listed coverage labels, reject the route as overclaimed.",
    ],
    "type_chart_navigation": [
