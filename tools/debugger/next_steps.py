...
EVIDENCE_STANDARDS = {
    ...
    "wrong_switch": [
        "A scenario JSONL matching the disputed switch case passes rom-switch-materialize on the current ROM with --fail-on-mismatch, checked_count > 0, and error_count == 0. The route names BossAI_SwitchOrTryItem, switch WRAM targets, the boss_policy_switch source anchors, and the scenario materialization command so the ordinary-language wrong-switch question is answered with a runnable proof. For switch_sack converter disagreement, rerun python -m tools.boss_ai_debugger rom-switch-materialize --scenarios <switch_sack_scenarios.jsonl> --limit 3 --fail-on-mismatch and name the scenario id, expected/proposed switch values, confidence, observation_status, switch_roll availability/frequency/probability_exact status, policy why text, ROM switch path, and stale-expectation-vs-ROM-bug status. Use --switch-threshold <byte> only when the final threshold is known externally and should be reported as exact; if switch_roll.available=false, say no final switch roll was observed instead of calling it a 0% switch chance.",
    ],
    "wrong_move_score": [
