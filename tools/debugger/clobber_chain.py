...
from .instruction_writes import (
    REGISTERS_8, REGISTER_PAIRS, pair_to_halves, normalize_dest,
    parse_instruction_line, clobbers_for_instruction,
)
...
def _scan_direct_clobbers(blocks: Iterable[FunctionBlock]) -> _DirectScan:
    ...
    # Only actual LIFO saves restored to the same pair protect intervening
    # writes. A cross-register pop is a transfer, not a restoration.
    pending_saves: list[tuple[str, int]] = []
    saved_at: dict[int, str] = {}
    restored_at: dict[int, str] = {}
    for index, line in enumerate(lines):
        ...
        if mnemonic == "push" and operands:
            pending_saves.append((normalize_dest(operands[0]), index))
        elif mnemonic == "pop" and operands and pending_saves:
            pair, start = pending_saves.pop()
            if pair == normalize_dest(operands[0]):
                saved_at[start] = pair
                restored_at[index] = pair
    for index, line in enumerate(lines):
        ...
        if parsed is None:
            continue
        mnemonic, operands = parsed
        if index in saved_at:
            for register in _expand_registers((saved_at[index],)):
                protected_counts[register] = protected_counts.get(register, 0) + 1
            continue
        if index in restored_at:
            for register in _expand_registers((restored_at[index],)):
