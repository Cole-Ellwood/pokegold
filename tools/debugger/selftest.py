...
def check_context_packet(root: Path) -> CheckResult:
    ...
    def inner() -> str:
        ...
        if not str(packet.get("markdown", "")).startswith("# Hypothesis context packet"):
            raise AssertionError("context packet did not render the standard header")
        return f"context packet valid and within budget ({packet.get('token_count')} tokens)"

    return _capture(
        component="context_packet",
        next_command="python -m tools.debugger pack --hypothesis <id>",
        fn=inner,
    )
