...
`tools.headless_battle` is the first text/JSON no-GUI battle-state path. It is intentionally narrow: selected move-vs-move turns, explicit stats or species shorthand, damage variation with fixed/sample/exhaustive RNG, pre-variation HP damage through the existing damage oracle, and post-score Boss AI selector replay.
