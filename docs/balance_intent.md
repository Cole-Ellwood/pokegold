...
| `MISDREAVUS` | provisional | boss-grade | Bulky Ghost attrition pivot for Morty and later Ghost teams; it should feel distinct from Gengar by buying turns for status, Pain Split, and Shadow Ball instead of racing as a sweeper. | Raw stats `100/80/90/85/70/95`; Morty uses level 25 Leftovers `CONFUSE_RAY` / `TOXIC` / `PAIN_SPLIT` / `SHADOW_BALL`; Ghost typing and support TM access. | Preserve the bulk/control identity; do not drift back toward high-Attack Haunter/Gengar overlap. | Playtest Morty's mid-fight pacing and whether Pain Split plus Leftovers creates pressure without becoming a stall wall. |
