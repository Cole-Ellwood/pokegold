...
Thunder Wave/Stun Spore/Glare paralysis-status moves with baseline paralyzed
speed/full-paralysis checks, repeated action maps with `max_turns`,
...
Fighting/Electric type-passive paralysis modifiers, Rest, weather/time healing,
drain moves, Heal Bell, RNG-consuming mechanics outside the listed branch
points, live Boss AI scoring, or scripts until the simulator report and audit
say that surface exists.
