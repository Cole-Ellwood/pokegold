...
| `boss_policy_switch.asm` | Switch dispatch (`BossAI_SwitchOrTryItem`); Switch threshold + loop penalty; Switch reason predicates; Switch-in classifiers; Ace timing + switch confidence; Switch-candidate risk refinement; Switch-confidence finalization |
