...
| **Spec-deviation by clever architecture (the ask-first gate)** | Spec mandates approach X (e.g., physical file partition with LOC targets); codex satisfies the load-bearing constraint (byte-identity, audits PASS) by inventing approach Y (e.g., an EMIT-guard pattern decoupling source organization from ROM emission order) without surfacing the deviation. Discovered post-merge during spot-check; an explicit acceptance criterion (LOC distribution) FAILs even though the deeper goal holds. The boss AI source repartition is the canonical example — see `docs/boss_ai_organization_plan.md` §3 Option C "EMIT-guard convention" for the architectural choice and its tradeoffs. | When codex's solution **changes the source organization model**, **adds an unusual build/include pattern**, or **knowingly misses an explicit acceptance criterion** (LOC distribution, spec-named partition shape, etc.) — STOP, surface the deviation as a one-paragraph question to the user describing the conflict and the alternative solution, ship only after explicit user approval. The rule fires when a hard constraint (`make compare` byte-identical) and an explicit acceptance criterion (LOC targets) collide and the architectural choice is not pre-authorized by the spec. Routine implementation detail decisions (variable naming, helper extraction within a function) do NOT trigger the gate. |
