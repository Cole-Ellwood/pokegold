...
| Boss AI live trace artifact | `python tools\trace\boss_ai_trace_capture.py --symbols-only`; `python tools\trace\boss_ai_trace_batch.py`; `python tools\audit\check_boss_ai_live_capture_ledger.py`; docs navigation; `git diff --check` | Trace ROM must match the state/artifact being claimed. | Old `.local/` RAM is not proof unless tied to the current trace ROM. |
