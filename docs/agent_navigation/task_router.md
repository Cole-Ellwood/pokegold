...
| "live proof", "trace", "gym leader/Koga/Lance/Red proof" | `docs/agent_navigation/subsystems/boss_ai_trace.md`, `docs/boss_ai_trace_capture.md`, `audit/boss_ai_trace/live_capture_ledger.md`, `audit/boss_ai_trace/live_capture_manifest.json` | `tools/trace/boss_ai_trace_state_probe.py`, `tools/trace/boss_ai_trace_capture.py`, `tools/trace/boss_ai_trace_batch.py`, trace ROM outputs | `python tools\trace\boss_ai_trace_capture.py --symbols-only`, Morty-only strict probe when the candidate is Morty, `python tools\trace\boss_ai_trace_batch.py`, `python tools\audit\check_boss_ai_live_capture_ledger.py` | Treating old `.local/` RAM as proof without matching current trace ROM state and passing any manifest-owned strict preflight. |
