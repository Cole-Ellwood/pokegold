...
| Morty does not pivot Misdreavus into Gengar on Haunter Ghost pressure | Static invariants confirm `BossAI_SwitchTargetSolvesDefensiveProblem` compares target risk against current-active risk before confidence rolls, but the specific live Morty situation still needs a real-fight feel check. | Fight Morty with Haunter against Misdreavus. Present a likely Ghost attack line and confirm Morty does not switch into Gengar unless a separate public exception applies. | pending |
| Boss avoids A→B→A switch-loop ping-pong | Static invariants and a synthetic switch-loop fixture (`audit/boss_ai_trace/shared_switch_loop_live.txt`) cover `BossAI_NeedsLoopPenalty`, but real-fight pivoting isn't traced. | Force a boss to switch A → B (e.g., scare it out with a super-effective threat), then on the next turn create conditions where A would be the obvious answer. Confirm the boss does not switch back to A unless a public emergency exception applies (meaningfully safer revenge switch, Perish Song escape, immunity pivot, ace timing). | pending |
...
| Map tiles "jumble" during overworld text scenes (give-item / heal) | Observed on 2026-05-01 and again on 2026-05-12 (VBA): visible map tiles randomize into garbage while a textbox is open, then snap back to correct when the textbox closes. Confirmed reports involved Nurse Joy heal, berry tree, Berry Master, starter receive/nickname flow, and one Mom day/DST prompt after receiving the Pokegear. Does NOT reproduce on plain signs / plain dialogue NPCs. RNG-flavored — same script triggers some attempts and not others. **Symptom shape implies `vBGMap` (on-screen tilemap) is being transiently overwritten while `wTileMap` (RAM source-of-truth) stays correct** — the close-text path's `wTileMap → vBGMap` push restores it. 2026-05-12 fix: `home/audio.asm` `WaitSFX` now clears BG map copying before and after SFX waits, addressing the textbox + jingle/fanfare overlap class and stale refresh carryover into the next prompt. See `docs/graphics_emulator_debugging.md` for the future triage route. | Reproduce: walk to a Pokemon Center, heal repeatedly until garble is observed (likely needs 3-10 attempts), then repeat starter receive/nickname and Mom day/DST after Pokegear if needed. When garble is on-screen: take a phone screenshot of the monitor for the garble pattern. Bonus: open VBA Tools → BG Map Viewer before triggering, pause emulator when garbled, screenshot the viewer window — that pinpoints which tilemap address got the bad write. | 2026-05-12 current ROM: starter nickname prompt pass on VBA after first `WaitSFX` fix; tightened after one Mom day/DST recurrence. Needs repeat VBA spot checks for Mom day/DST plus older heal/give-item scenes |
...
| Ditto Imposter auto-Transforms on battle entry | `engine/battle/core.asm` calls `TryActivateDittoImposter` after initial leads are loaded and after hazard processing on switch-in. Activation text is not runtime-tested across edge cases (Fly/Dig hidden opponent should suppress; already-transformed Ditto should not retrigger). | Encounter a wild Ditto as the enemy lead; confirm the Imposter text and Transform text fire automatically after the player lead appears. Switch a player Ditto into a wild battle; confirm it auto-Transforms. Repeat into an opponent mid-Fly/Dig (Ditto should not Transform until they reappear). Send Ditto out twice in the same battle; confirm it does not re-Transform after the first. | pending |
