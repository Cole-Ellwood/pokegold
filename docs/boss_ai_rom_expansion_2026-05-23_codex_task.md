...
- **Resolved 2026-05-25** — Player-first Haki sequencing now fires immediately before the enemy action after the player's move or switch-in has resolved. The unresolved future variant would be a pre-player-animation interrupt, which is not current design.
