...
- **Resolved follow-up** — Player-first Haki sequencing is handled by the pre-order hook after `ParsePlayerAction` and before `DetermineMoveOrder`; Haki is not gated on enemy-first order.
