...
| **N. Per-tick memo caches** | `wBossAIHasKOMoveCache`, `wBossAIPublicThreatCache`, `wBossAIRevealedPriorityCache`, `wBossAIPrimaryThreatCache`, `wBossAIPublicEnemyFasterCache`, `wBossAILookaheadDepthCache`, `wBossAITransformSource` (the retired `wBossAILookaheadRunningBest` byte, since 2026-09-08 the player's Transform memory rather than a per-tick cache), `wBossAILastMatchupType`, `wBossAILastMatchupResult`, `wBossAIShouldScoutPrereqCache`, `wBossAIShouldScoutThresholdCache`, `wBossAIShouldScoutMatchupValue` | 12 | 8.6 | Sentinel-bit per-tick caches that collapse repeated heavy queries from inside lookahead |
