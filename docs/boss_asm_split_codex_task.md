...
| `boss_policy_switch.asm` | Switch dispatch (`BossAI_TrySwitch` + base candidate scan); Switch threshold + loop penalty; Switch reason predicates; Switch-in classifiers; Ace timing + switch confidence; Switch-candidate risk refinement; Switch-confidence finalization. |
