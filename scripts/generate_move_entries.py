...
def parse_names(constants: list[str]) -> dict[str, str]:
    ...
    if len(names) != len(constants):
        raise ValueError(
            f"move name count mismatch: {len(names)} names for {len(constants)} constants"
        )
    out: dict[str, str] = {}
    for constant, raw_name in zip(constants, names):
        out[constant] = NAME_OVERRIDES.get(constant, title_move_constant(raw_name.replace("-", "_")))
    return out
...
def stat_change_summary(effect: str, move: Move) -> str | None:
    ...
    if hit_match:
        ...
        return f"It has a {chance_text(move)} chance to lower the target's {stat} by one stage."

    hit_match = re.match(r"EFFECT_(ATTACK|DEFENSE|SPEED|SP_ATK|SP_DEF|ACCURACY|EVASION)_UP_HIT$", effect)
    if hit_match:
        stat = STAT_NAMES[hit_match.group(1)]
        return f"It has a {chance_text(move)} chance to raise the user's {stat} by one stage."
...
def effect_summary(move: Move) -> list[str]:
    ...
    if effect in HIT_STATUS_EFFECTS:
        ...
        return [f"It has a {chance_text(move)} chance to {verb}."]
...
def render_entry(
    ...
    *,
    ...
    critical_moves: set[str],
    priorities: dict[str, int],
) -> list[str]:
    lines = [f"## {move.display_name}", ""]
    paragraphs = [first_sentence(move, type_values, special_threshold)]
    ...
    paragraphs.append(" ".join(summary_sentences + extra_sentences))
